...
/**
 *Project Overview:
 * The project entails a GUI application for creating, manipulating, and visualizing
 * geometric shapes using the StdDraw library. Central to the project is the Ex2
 *  controller, orchestrating interactions with various shapes.
 *
 *Geometric Shapes:
 * Implemented within the 'ex2.geo' package, the project offers a range of 2D
 * geometric shapes such as Circle, Segment, Polygon, Triangle, and Rectangle.
 * These shapes support fundamental operations like translation, rotation, and scaling.
 *
 *GUI Management:
 * The 'ex2.gui' package manages the GUI, utilizing StdDraw_Ex2 for shape
 * rendering on the canvas. Clear interfaces like Ex2_GUI, GUI_Shape, and
 * GUI_Shape_Collection guide class implementations.
 *
 *Project Goals:
 * The aim is to provide users with a user-friendly interface to draw shapes,
 * perform manipulations (move, copy, rotate, scale), customize properties (color,
 * fill), and execute actions like sorting based on various criteria.
 *
 *Key Features:
 * Singleton Design Pattern: Ensures a single instance of the Ex2 controller for efficiency.
 * File I/O Functionality: Enables saving and loading of shape collections for persistence.
 * Mouse Interactions: Supports shape creation and manipulation via clicks and movements.
 * Sorting Functionality: Allows sorting of shapes based on area, perimeter, and other criteria.
 * Service Methods: Includes display, shape information retrieval, and initialization.
 *
 *Conclusion:
 * In summary, the project delivers an efficient tool for interacting with geometric shapes via a
 * user-friendly GUI, offering essential functionalities for shape management and visualization.
 *
...
public class Ex2 implements Ex2_GUI {
    ...
    /**
     * Initializes the ShapeCollection and other fields based on the provided GUI_Shape_Collection.
     * @param s The GUI_Shape_Collection to initialize from.
     */
    ...
    /**
     * Sets the scale of StdDraw_Ex2, shows the canvas, and draws shapes on it.
     * @param d The scale to set for the canvas.
     */
    ...
    /**
     * Retrieves the singleton instance of the Ex2 class.
    ...
    /**
     * Draws all shapes in the _shapes collection and shows the canvas.
     */
    ...
    /**
     * Draws the specified GUI_Shape on the canvas.
    ...
    /**
     * Sets the color of selected shapes within the _shapes collection to the specified color.
    ...
    /**
     * Sets the fill status of selected shapes within the _shapes collection.
     * The fill status is determined by the boolean value _fill.
     */
    ...
    /**
     * Removes selected shapes from the _shapes collection.
     */
    private void remove() {
    ...
    /**
     * Handles various actions based on the provided string p.
     * @param p The string representing the action to perform.
     */
    ...
    /**
     * Saves the current state of the shapes to a text file.
     */
    private void save() {
    ...
    /**
     * Scales selected shapes around a specified pivot point by a given ratio.
     * @param p The pivot point for scaling.
    ...
    /**
     * Selects or deselects shapes based on whether the given point is contained within them.
     * @param p The point to check for selection.
     */
    ...
    /**
     * Moves selected shapes by a specified translation vector.
     */
    private void move() {
        for (int i = 0; i < _shapes.size(); i++) {
            GUI_Shape s = _shapes.get(i);
    ...
    /**
     * Duplicates selected shapes and places the copies at a new position defined by the translation vector.
     */
    private void copy() {
        for (int i = 0; i < _shapes.size(); i++) {
            GUI_Shape s = _shapes.get(i);
    ...
    /**
     * Rotates selected shapes around a pivot point.
     * @param ang The current mouse position for calculating the rotation angle.
     */
    ...
    /**
     * Toggles the selection state of all shapes in the collection.
     * If a shape is selected, it will be deselected, and vice versa.
     */
     ...
     * This method finalizes the creation of a polygon (if in Polygon mode),
     * resets various variables, and updates the display.
     * @param p The point where the right-click event occurred.
     */
     ...
     * This method tracks the movement of the mouse cursor and updates the display
     * to visualize the ongoing drawing operation.
    ...
    /**
     * Retrieves the collection of GUI shapes stored in this object.
     * @return The GUI_Shape_Collection containing the shapes.
     */
    @Override
    ...
    /**
     * Shows the GUI shapes using the default dimension size defined in Ex2_Const.
     */
    @Override
    public void show() {
    ...
    /**
     * Retrieves information about the shapes in the collection.
     * Each shape's string representation is appended to a single string, separated by newline characters.
     * @return A string containing the string representations of all shapes in the collection.
     */
    @Override
